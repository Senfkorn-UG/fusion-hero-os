...
echo "[1/10] README/Mesh — bereits in mesh_roles.yaml verankert"
...
echo ""
echo "[2/10] Verification LLM-Recovery Status"
...
echo ""
echo "[8/10] GDrive Storage Policy (nicht-operative Daten)"
if command -v powershell.exe >/dev/null 2>&1; then
...
echo ""
echo "[9/10] Bottom-Up Merge (WSL -> Windows -> GitHub)"
if command -v powershell.exe >/dev/null 2>&1; then
...
echo ""
echo "[10/10] Grok CLI + Skill-Sync (Windows)"
if command -v powershell.exe >/dev/null 2>&1; then
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$ROOT/workstation/check-grok-cli.ps1" 2>&1 | sed 's/^/  /' || true
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$ROOT/workstation/sync_grok_intern.ps1" 2>&1 | sed 's/^/  /' || true
else
  echo "  powershell.exe nicht verfuegbar"
fi
