# OS-level Mainframe Daemons (Kostenanalyse + Repo-Spiegelung/Korrektur)
...
print('[mainframe-daemon] cost + repo mirror started', flush=True)
while cost._running and mirror._running:
    try:
        cost.tick()
...
Write-Host "Mainframe OS daemons gestartet (Kosten ${CostIntervalSec}s, Mirror ${MirrorIntervalSec}s)"
