...
Write-Host "[1/10] Mesh Mainframe-Rolle" -ForegroundColor Yellow
...
Write-Host "[2/10] Verification API" -ForegroundColor Yellow
try {
...
Write-Host "[3/10] Tailscale + Mesh" -ForegroundColor Yellow
& "$PSScriptRoot\status.ps1" | Select-Object -First 20

Write-Host "[4/10] AudioRelay" -ForegroundColor Yellow
& "$PSScriptRoot\check-audio-relay.ps1"
& "$PSScriptRoot\route-audio-to-phone.ps1"

Write-Host "[5/10] Gmail Triage" -ForegroundColor Yellow
python (Join-Path $Root "scripts\gmail_triage_bridge.py") 2

Write-Host "[6/10] Archiv-Anker" -ForegroundColor Yellow
python (Join-Path $Root "scripts\archiv_anchor_uncommitted.py") --include-ignored

Write-Host "[7/10] Planova Inneneinrichter" -ForegroundColor Yellow
...
Write-Host "[8/10] GDrive Storage Policy" -ForegroundColor Yellow
& (Join-Path $PSScriptRoot "apply-storage-policy.ps1")

Write-Host "[9/10] Bottom-Up Merge" -ForegroundColor Yellow
...
Write-Host "[10/10] Grok CLI + Skill-Sync" -ForegroundColor Yellow
& (Join-Path $PSScriptRoot "check-grok-cli.ps1")
& (Join-Path $PSScriptRoot "sync_grok_intern.ps1")
$grokStatus = Join-Path $env:USERPROFILE ".fusion\grok-cli.status.json"
if (Test-Path $grokStatus) {
    $gs = Get-Content $grokStatus -Raw | ConvertFrom-Json
    Write-Host "  Grok: v$($gs.version) | skill_ok=$($gs.skill_ok) | HEAD=$(git -C $Root rev-parse --short HEAD 2>$null)" -ForegroundColor $(if ($gs.ok) { "Green" } else { "Yellow" })
}
