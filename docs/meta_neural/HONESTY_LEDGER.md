...
| PII gate private denylist hook (device ids / legal names / company) | **Implemented** | Denylist patterns loaded from a git-ignored file (`scripts/pii_denylist.local.yaml` or `$FUSION_PII_DENYLIST`); blocking, never allowlist-exempt, fail-closed. No real PII in source. `::test_denylist_catches_device_and_name_and_company`, `::test_denylist_hits_bypass_allowlist`, `::test_denylist_missing_env_path_fails_closed`, `::test_denylist_source_contains_no_real_pii` |
