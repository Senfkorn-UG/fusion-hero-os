...
- **Hyper-Threading** — `03_Code/core/virtual_gpu_hyperthreading.py`'s `VirtualGPUHTCache` ist **selbst-dokumentiert als Simulation** ("Simulates hyper-parallel threads"), keine native Hardware-Fähigkeit. (Separat davon: `fusion_hero_os/engine/mainframe.py` hat eine echte, gemessene Mehrkern-Parallelisierung — `backend="auto"` -> Rust/rayon ~4.4x bzw. Numba-nogil ~3.4x — und ist seit 2026-07-04 der aktive GUI-Pfad; nicht mit der Simulation verwechseln.)
