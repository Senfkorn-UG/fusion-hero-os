...
try:  # pragma: no cover - defensiv, im Projekt vorhanden
    from methodology.connectors import BaseConnector
except Exception:  # pragma: no cover - Fallback, falls als Skript aus engine/ gestartet
...
def optimize_switching(
    ...
    """
    try:
        from engine.mainframe import parallel_anneal
    except Exception:  # pragma: no cover - Skriptstart aus engine/
        from mainframe import parallel_anneal  # type: ignore[no-redef]
