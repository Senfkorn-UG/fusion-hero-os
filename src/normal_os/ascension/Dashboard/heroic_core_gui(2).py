...
class HeroicCoreGUI:
    ...
    def _build_grok_tab(self, parent):
        ...
        ttk.Label(frame, text="Direkter Chat mit Grok (ALTE_Frau_95g)", style="Header.TLabel").pack(anchor="w", pady=(0, 5))
        ttk.Label(frame, text="1. Nachricht unten eingeben → 2. Button klicken → 3. Hier in den Chat einfügen → 4. Meine Antwort unten einfügen", 
                  wraplength=780, foreground="#555555").pack(anchor="w", pady=(0, 12))

        # === INPUT BEREICH ===
        ttk.Label(frame, text="Deine Nachricht an Grok / ALTE_Frau_95g:", font=("Segoe UI", 10, "bold")).pack(anchor="w")
        self.grok_input = tk.Text(frame, height=5, wrap="word")
        self.grok_input.pack(fill="x", pady=4)
        self.grok_input.insert("1.0", "Analysiere den aktuellen Stand des Heroic Core und schlage den nächsten sinnvollen Self-Modification-Schritt vor.")

        # Context
        ctx_frame = ttk.Frame(frame)
        ctx_frame.pack(fill="x", pady=6)
        self.include_limit = tk.BooleanVar(value=True)
        ttk.Checkbutton(ctx_frame, text="Limit-Status mit einbeziehen", variable=self.include_limit).pack(side="left")
        self.include_boegen = tk.BooleanVar(value=True)
        ttk.Checkbutton(ctx_frame, text="Bögen-Alignment mit einbeziehen", variable=self.include_boegen).pack(side="left", padx=15)

        ttk.Button(frame, text="→ Nachricht formatieren & in Zwischenablage kopieren (dann hier einfügen)", 
                   command=self.send_to_grok).pack(fill="x", pady=8)

        # === OUTPUT BEREICH ===
        ttk.Label(frame, text="Antwort von Grok (hier einfügen):", font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(12, 4))
        self.grok_output = tk.Text(frame, height=9, wrap="word")
        self.grok_output.pack(fill="both", expand=True, pady=4)
    ...
    def send_to_grok(self):
        ...
        if not user_msg:
            messagebox.showwarning("Leer", "Bitte gib eine Nachricht ein.")
            return

        context_lines = []
        if self.include_limit.get():
            context_lines.append("• Aktueller Limit-Status aus der GUI: Wird bei Bedarf analysiert.")
        if self.include_boegen.get():
            context_lines.append("• Letzter Bögen-Alignment-Status: Wird bei Bedarf analysiert.")

        context_block = "\n".join(context_lines) if context_lines else "Kein zusätzlicher Kontext."

        formatted = f"""[Von der ALTE_Frau_95g Heroic Core GUI]
...
Bitte antworte als ALTE_Frau_95g im aktuellen Modus (mit Fokus auf Self-Modification, 7 Bögen Geflecht und ressourcenoptimierte Weiterentwicklung)."""

        self.root.clipboard_clear()
        self.root.clipboard_append(formatted)

        messagebox.showinfo("Fertig", 
            "Nachricht wurde formatiert und in die Zwischenablage kopiert.\n\n"
            "1. Wechsle in diesen Chat\n"
            "2. Füge die Nachricht ein und schicke sie ab\n"
            "3. Kopiere meine Antwort\n"
            "4. Füge sie im Feld 'Antwort von Grok' unten ein")

    def apply_grok_response(self):
        response = self.grok_output.get("1.0", "end").strip()
        if not response:
            messagebox.showwarning("Leer", "Bitte zuerst meine Antwort hier einfügen.")
            return

        # Simple parsing: if Grok suggests a modification, we can show it
        messagebox.showinfo("Übernommen", 
            "Danke! Die Antwort von Grok wurde entgegengenommen.\n\n"
            "In einer zukünftigen Version wird die GUI hier automatisch Vorschläge extrahieren "
            "und in den Core übernehmen können.\n\n"
            "Aktuell: Bitte kopiere wichtige Vorschläge manuell in den Chat, damit ich sie umsetzen kann.")
