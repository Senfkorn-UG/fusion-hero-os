...
# Middle-out integration: bring springloop energy for contraction
sys.path.insert(0, str(Path(__file__).parent.parent))
try:
    from core.qb_qubo import springloop_energy, energy as qubo_energy
    HAS_SPRINGLOOP = True
except Exception:
    HAS_SPRINGLOOP = False
    def springloop_energy(Q, x, **k): return x, 0.0
    def qubo_energy(Q, x): return 0.0
...
class GeisterjagdBanachViz:
    ...
    def _banach_step(self, hints: Dict[str, Any]) -> None:
        ...
        # Middle-out: optional springloop energy contraction (if available)
        use_spring = hints.get("use_springloop", False) and HAS_SPRINGLOOP
        if use_spring:
            # Map 2D state to small QUBO for springloop demo
            Q = [[0.5, 0.1], [0.1, 0.6]]  # simple quadratic
            x = [self.s_n[0], self.s_n[1]]
            xs, es = springloop_energy(Q, x, steps=5, k=0.4, damping=0.9)
            self.s_n = [float(xs[0]), float(xs[1])]
