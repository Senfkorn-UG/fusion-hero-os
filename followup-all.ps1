...
Write-Host "[1/8] Mesh Mainframe-Rolle" -ForegroundColor Yellow
...
Write-Host "[2/8] Verification API" -ForegroundColor Yellow
try {
...
Write-Host "[3/8] Tailscale + Mesh" -ForegroundColor Yellow
& "$PSScriptRoot\status.ps1" | Select-Object -First 20

Write-Host "[4/7] AudioRelay" -ForegroundColor Yellow
& "$PSScriptRoot\check-audio-relay.ps1"
& "$PSScriptRoot\route-audio-to-phone.ps1"

Write-Host "[5/7] Gmail Triage" -ForegroundColor Yellow
python (Join-Path $Root "scripts\gmail_triage_bridge.py") 2

Write-Host "[6/7] Archiv-Anker" -ForegroundColor Yellow
python (Join-Path $Root "scripts\archiv_anchor_uncommitted.py") --include-ignored

Write-Host "[7/7] Planova Inneneinrichter" -ForegroundColor Yellow
