...
from orchestration import controller_aggregate as ctrl  # Owner-Ebene + Layer-Aggregat
...
# --- Controller Aggregat (Endnutzer-Schnittstelle, Owner-Ebene) -----------
CONTROLLER = ctrl.ControllerAggregate()
owner_session = {"token": None}

# Eingebaute Capabilities: Anwendungs-Startcode registriert ohne Token
# (siehe bootstrap_register-Docstring — kein UI/API-Pfad ruft das auf).
CONTROLLER.bootstrap_register(ctrl.Capability(
    "qubo_mainframe", layer=1,
    description="HEROIC Mainframe · QUBO-Solver-Dialog",
    loader=lambda: "verfügbar über den Memory-Icon-Button"))
CONTROLLER.bootstrap_register(ctrl.Capability(
    "hauptagent_chat", layer=2,
    description="Hauptagent Chat-I/O (Multi-Agent-Orchestrierung)",
    loader=lambda: "verfügbar im Hauptagent-Panel"))
CONTROLLER.bootstrap_register(ctrl.Capability(
    "pipelines", layer=3,
    description="Workflow-Pipelines (Erkenntnis-Lauf, Sweep, Review, Parallel-Solve)",
    loader=lambda: "verfügbar im Pipelines-Panel"))
CONTROLLER.bootstrap_register(ctrl.Capability(
    "live_metrics", layer=4,
    description="CPU/RAM-Live-Metriken + Hauptagent-Monitor",
    loader=lambda: "läuft automatisch (ui.timer)"))
...
# =========================================================================
# Controller Aggregat — Layer-Übersicht (öffentlich) + Owner-Login (Token)
# =========================================================================
def render_controller_layers():
    ctrl_layers_col.clear()
    with ctrl_layers_col:
        for l in CONTROLLER.describe_layers():
            active = l["active"]
            with ui.row().classes("items-center w-full gap-2 flex-nowrap"):
                ui.icon("radio_button_checked" if active else "radio_button_unchecked", size="14px") \
                    .classes("text-emerald-400" if active else "text-[#475569]")
                ui.label(f"L{l['level']} · {l['name']}").classes(
                    f"text-xs font-mono w-40 {'text-[#e2e8f0]' if active else 'text-[#94a3b8]'}")
                ui.label(l["description"]).classes("text-[11px] text-[#94a3b8] flex-1")


def render_controller_capabilities():
    ctrl_caps_col.clear()
    with ctrl_caps_col:
        active_names = {c.name for c in CONTROLLER.resolve_active()}
        caps = sorted(CONTROLLER.list_all(), key=lambda c: c.layer)
        if not caps:
            ui.label("— keine Capabilities registriert —").classes("text-xs text-[#475569]")
        for c in caps:
            is_active = c.name in active_names
            with ui.row().classes("items-center w-full gap-2 flex-nowrap"):
                ui.label("● implizit" if is_active else "○ manuell").classes(
                    f"text-[10px] w-16 {'text-emerald-400' if is_active else 'text-[#475569]'}")
                ui.label(f"L{c.layer}").classes("text-[10px] text-[#94a3b8] font-mono w-8")
                ui.label(c.name).classes("text-xs text-[#e2e8f0] w-40 truncate")
                ui.label(c.description).classes("text-[11px] text-[#94a3b8] flex-1 truncate")


def render_owner_panel():
    owner_panel_col.clear()
    with owner_panel_col:
        if owner_session["token"] and CONTROLLER.auth.verify(owner_session["token"]):
            ui.label("✓ angemeldet als Owner").classes("text-xs text-emerald-400")
            with ui.row().classes("items-center gap-2 flex-wrap"):
                thr_sel = ui.select(
                    {l.level: f"L{l.level} · {l.name}"
                     for l in sorted(CONTROLLER.layers.values(), key=lambda x: x.level)},
                    value=CONTROLLER.active_threshold(), label="Implizitheits-Schwellwert") \
                    .props("dense dark options-dense").classes("w-64")

                def apply_threshold():
                    try:
                        CONTROLLER.set_threshold(int(thr_sel.value), owner_session["token"])
                        render_controller_layers()
                        render_controller_capabilities()
                        ui.notify(f"Schwellwert → L{thr_sel.value}", type="positive")
                    except (ctrl.OwnerAuthError, ValueError) as exc:
                        ui.notify(str(exc), type="negative")

                ui.button("Anwenden", on_click=apply_threshold).props("dense color=teal")

                def do_logout():
                    CONTROLLER.auth.revoke(owner_session["token"])
                    owner_session["token"] = None
                    render_owner_panel()
                    ui.notify("Abgemeldet", type="info")

                ui.button("Abmelden", on_click=do_logout).props("flat dense color=red")
        elif not CONTROLLER.auth.is_initialized:
            ui.label("Owner noch nicht eingerichtet — einmaliges Setup.").classes(
                "text-xs text-amber-400")
            pw1 = ui.input("Neues Owner-Passwort (≥ 8 Zeichen)", password=True) \
                .props("dense dark").classes("w-64")

            def do_init():
                try:
                    CONTROLLER.auth.initialize_owner(pw1.value or "")
                    pw1.value = ""
                    ui.notify("Owner eingerichtet — jetzt anmelden.", type="positive")
                    render_owner_panel()
                except (ctrl.OwnerAuthError, ValueError) as exc:
                    ui.notify(str(exc), type="negative")

            ui.button("Owner einrichten", on_click=do_init).props("dense color=teal")
        else:
            with ui.row().classes("items-center gap-2 flex-wrap"):
                pw = ui.input("Owner-Passwort", password=True).props("dense dark").classes("w-64")

                def do_login():
                    token = CONTROLLER.auth.authenticate(pw.value or "")
                    pw.value = ""
                    if token:
                        owner_session["token"] = token
                        render_owner_panel()
                        ui.notify("Angemeldet", type="positive")
                    else:
                        ui.notify("Falsches Passwort", type="negative")

                pw.on("keydown.enter", do_login)
                ui.button("Anmelden", on_click=do_login).props("dense color=teal")
...
with ui.column().classes("w-full h-screen flex-nowrap gap-0 p-0"):
    ...
    with ui.row().classes("w-full items-center gap-3 px-4 h-12 bg-[#0d0d14] "
        ...
        ui.button(icon="hub", on_click=lambda: controller_panel.open()).props("flat color=teal") \
            .tooltip("Controller Aggregat · Layer & Owner")
    ...
    # Controller Aggregat (einklappbar) — Endnutzer-Schnittstelle: Layer + Owner-Login
    with ui.expansion("Controller Aggregat", icon="hub", value=False) \
            .classes("w-full bg-[#0d0d14] border-t border-[#1e1e2e]") as controller_panel:
        with ui.column().classes("w-full p-2 gap-3"):
            ui.label("Layer 0 (explizit) → Layer 5 (ambient) — je höher, desto impliziter geladen.") \
                .classes("text-[11px] text-[#94a3b8]")
            ctrl_layers_col = ui.column().classes("w-full gap-1")
            ui.label("REGISTRIERTE CAPABILITIES").classes(
                "text-[10px] font-bold text-[#475569] tracking-wider mt-1")
            ctrl_caps_col = ui.column().classes("w-full gap-1")
            ui.label("OWNER-BEREICH").classes(
                "text-[10px] font-bold text-[#475569] tracking-wider mt-2")
            owner_panel_col = ui.column().classes("w-full gap-2")
