#!/usr/bin/env python3
"""Export private knowledge base into JSONL training data for local LLM."""
...
SOURCES = [
    Path(__file__).parent.parent,  # scan the private-hacking-suite itself
    # Add more local paths here if needed, e.g. other private folders
]
...
def samples_from_python(path: Path, text: str) -> list[dict]:
    ...
    name = path.stem
    samples = [{
        "prompt": f"Wofür ist das Modul {name}?",
